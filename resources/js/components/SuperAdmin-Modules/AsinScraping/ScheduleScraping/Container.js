...
import ExportCsv from '../../../../general-components/ExportCsv';
import ScheduleTime from '../../SearchRankScraping/ScheduleScraping/ScheduleTime';
import ScheduleTimeLoader from '../../SearchRankScraping/ScheduleScraping/ScheduleTime/ScheduleTimeLoader';
...
class Container extends Component {
    ...
    componentDidMount() {
        ...
        getAllSchedule((response)=>{
            this.setState({
                ...
                modal:{
                    open:false,
                    modalComponent:<AddSchedule id={0} handleModalClose = {this.handleModalClose} heloperReloadDataTable = {this.helperReloadDataTable} collections={this.state.collections}/>,
    ...
    filterOrignalData =(value) =>{
        return this.state.orignalData.filter(row => {
            ...
            row.cronStatus.toString().toLowerCase().includes(value.toLowerCase()) ||
            row.lastRun.toLowerCase().includes(value.toLowerCase())||
            row.cronDuration.toLowerCase().includes(value.toLowerCase())||
    ...
    render() {
        ...
        return (
            <>
                <Helmet>
                    <title>Ad-Tech | Asin Scraping</title>
                </Helmet>
                <div style={{display: 'table', tableLayout:'fixed', width:'100%'}} className="ASINCollections">
                      <div className="flex justify-between scheduleControls w-full">
                        {this.state.scheduleTime != "" ? <ScheduleTime 
                            scheduleTime = {this.state.scheduleTime}
                            url = "/settings/SchedulerTime"
                        /> : <ScheduleTimeLoader />}
                        <ExportCsv 
                            checkDataUrl = "/checkHistory"
                            className="w-2/6"
                        />
                    </div>
