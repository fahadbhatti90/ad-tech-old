...
class eventTracking extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'eventTrackingCron:event {activeCollections*}';
     ...
     * @var string
     */
    protected $description = 'This Command is used to log event of price change, review change etc..';
    ...
    {
        $logData = [];
        // pass current day date due to check is asin data is inserted or not
        $Data = $this->findPreviousDayData($asinValue, date('Y-m-d'), 3);
        if (empty($Data)) {
        ...
        switch ($eventType) {
            ...
            case 3:
                if(!empty($collectionIds)){
                    $activeAisnList = asinModel::whereIn("c_id", $collectionIds)
                        ->with("activeAsin")
                        ->get();
                    $activeAisnListAsin = collect($activeAisnList)->map(function ($item) {
                        if ($item->activeAsin) {
                            return ['accountId' => $item->activeAsin->accountId, 'asin' => $item->activeAsin->asin];
                        }
                    })->reject(function ($item) {
                        return empty($item);
                    });
                    return $activeAisnListAsin;
                }
            break;
            default:
                return ScrapModel::select('id','c_id','url','asin','asinExists','capturedAt','metaKeywords','brand','offerPriceOrignal','offerPrice','listPriceOrignal','listPrice',
                    ...
                    'shipWeight','dateFirstAvailable','fkAsinId')->with('getAsinAccounts.accounts:id,accountName')
                    ->groupBy('asin')
                    ->where('capturedAt', $capturedDate)
                    ->get();
            break;
    ...
    public function findPreviousDayData($asin, $previousDate, $eventType)
    {
        switch ($eventType) {
            case 3:
                return ScrapModel::select('tbl_asins_result.*')
                    ->where('tbl_asins_result.asin', $asin)
