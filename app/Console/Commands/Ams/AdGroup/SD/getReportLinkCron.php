...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsdadgroupreportlink:sdadgroup';
     ...
     * @var string
     */
    protected $description = 'This command is used to generate Sponsored Display AdGroup Report location.';
    ...
    {
        Log::info("filePath:Commands\Ams\AdGroup\SD\getReportLinkCron. Start Cron.");
        Log::info($this->description);
                $reportType = 'AdGroup_SD';
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $addReportIdObj->addSDAdGroupReportDownloadLink($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : Sponsored Display AdGroup Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : Sponsored Display AdGroup Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                Log::error('Refresh Access token. In file filePath:Commands\Ams\AdGroup\SD\getReportLinkCron.');
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('SD_AdGroup Report Link', 'fail');
