...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsbadgroupreportlink:sbadgroup';
    ...
    {
        Log::info("filePath:Commands\Ams\AdGroup\SB\getReportLinkCron. Start Cron.");
        Log::info($this->description);
                $reportType = 'AdGroup_SB';
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $addReportIdObj->addSBAdGroupReportDownloadLink($storeArray);
                                // store report status
                            ...
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:Commands\Ams\AdGroup\SB\getReportLinkCron.');
        ...
        Log::info("filePath:Commands\Ams\AdGroup\SB\getReportLinkCron. End Cron.");
    }
}
