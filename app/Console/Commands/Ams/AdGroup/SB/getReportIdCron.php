...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsbadgroupreportid:sbadgroup';
    ...
    {
        Log::info("filePath:Commands\Ams\AdGroup\SB\getReportIdCron. Start Cron.");
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    ...
                    $url = Config::get('constants.amsApiUrl') . '/' . Config::get('constants.apiVersion') . '/' . Config::get('constants.adGroupsReportSb');
                    $client = new Client();
                    b:
                        ...
                        try {
                            ...
                            $response = $client->request('POST', $url, [
                                ...
                                'json' => [
                                    ...
                                    'metrics' => Config::get('constants.sbAdGroupMetrics')],
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $storeArray['reportType'] = "AdGroup_SB";
                            ...
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:Commands\Ams\AdGroup\SB\getReportIdCron');
        ...
        Log::info("filePath:Commands\Ams\AdGroup\SB\getReportIdCron. End Cron.");
    }
}
