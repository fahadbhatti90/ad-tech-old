...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getcampaignreportlink:sdcampaign';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Campaign SD Report Location.';
    ...
    {
        Log::info("filePath:Commands\Ams\Campaign\SD\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        $reportType = 'Campaign_SD';
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $addReportIdObj->addcampaignReportLocationSD($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : Campaign SD Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : Campaign SD Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:Commands\Ams\Campaign\SD\getReportLinkCron');
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('Campaign SD Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\Campaign\SD\getReportLinkCron. End Cron.");
    }
}
