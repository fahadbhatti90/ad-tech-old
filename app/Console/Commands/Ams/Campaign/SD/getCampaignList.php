...
class getCampaignList extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getCampaignList:campaignSD';
        ...
        Log::info("Auth token get from DB Start!");
                ...
                if (!empty($allProfileIds)) {
                    foreach ($allProfileIds as $single) {
                        ...
                        $url = Config::get('constants.amsApiUrl') . '/' . Config::get('constants.sdCampaignUrl');
                        ...
                        try {
                            ...
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:App\Console\Commands\Ams\Campaign\SD\getCampaignList');
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('App\Console\Commands\Ams\Campaign\SD\getCampaignList', 'fail');
