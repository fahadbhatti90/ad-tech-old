                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $addReportIdObj->addcampaignReportLocation($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : Campaign SP Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : Campaign SP Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('Campaign SP Report Link', 'fail');
