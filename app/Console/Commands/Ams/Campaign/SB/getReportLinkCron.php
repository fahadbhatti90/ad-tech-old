...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getcampaignreportlink:sbcampaign';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Campaign SB Report Location.';
    ...
    {
        Log::info("filePath:Commands\Ams\Campaign\SB\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        $reportType = 'Campaign_SB';
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $addReportIdObj->addcampaignReportLocationSB($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : Campaign SB Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : Campaign SB Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('Campaign SB Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\Campaign\SB\getReportLinkCron. End Cron.");
    }
}
