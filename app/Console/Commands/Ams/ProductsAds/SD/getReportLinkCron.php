...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsdproductsadsreportlink:sdproductsads';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Sponsored Display Product Ads Report Link location.';
    ...
    {
        Log::info("filePath:Commands\Ams\ProductsAds\SD\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        $reportType = 'SD_Product_Ads';
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body)) {
                                ...
                                $addReportIdObj->addSDproductsAdsReportLocation($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : Sponsored Display ProductAds Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : Sponsored Display ProductAds Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                                Log::error('Refresh Access token. In file filePath:Commands\Ams\SD\ProductsAds\getReportLinkCron');
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('Sponsored Display ProductAds Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\ProductsAds\SD\getReportLinkCron. End Cron.");
    }
}
