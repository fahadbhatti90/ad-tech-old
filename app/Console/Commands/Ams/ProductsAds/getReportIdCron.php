...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getproductsadsreportid:productsads';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Product Ads ReportId.';
    ...
    {
        Log::info("filePath:Commands\Ams\ProductsAds\getReportIdCron. Start Cron.");
        ...
        if (!empty($AllProfileID)) {
            foreach ($AllProfileID as $single) {
                ...
                if (!empty($clientId)) {
                    $body = array();
                    $MetricListType = Config::get('constants.productAdsMetrics');
                    if ($single->type != 'vendor') {
                        $MetricListType = Config::get('constants.productAdsMetricsSKU');
                    }
                    // Create a client with a base URI
                    $url = Config::get('constants.amsApiUrl') . '/' . Config::get('constants.apiVersion') . '/' . Config::get('constants.productAdsReport');
                    $client = new Client();
                    b:
                    ...
                    if (!empty($accessToken)) {
                        try {
                            ...
                            if (!empty($body)) {
                                ...
                                $storeArray['reportType'] = "Product_Ads";
                            ...
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:Commands\Ams\ProductsAds\getReportIdCron');
        ...
        Log::info("filePath:Commands\Ams\ProductsAds\getReportIdCron. End Cron.");
    }
}
