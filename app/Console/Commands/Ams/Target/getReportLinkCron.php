...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportlink:targets';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\getReportLinkCron. Start Cron.");
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body)) {
                                ...
                                $addReportIdObj->addSpTargetsReportDownloadLink($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : Product Target Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : Product Target Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\getReportLinkCron');
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('Product Target Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\Target\getReportLinkCron. End Cron.");
    }
}
