...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getasinreportlink:asinreport';
     ...
     * @var string
     */
    protected $description = 'This command is used to generate ASIN Report location.';
    ...
    {
        Log::info("filePath:Commands\Ams\ASIN\getReportLinkCron. Start Cron.");
                    ...
                    foreach ($AllGetReports as $single) {
                        ...
                        try {
                            ...
                            if (!empty($body) && $body != null) {
                                ...
                                $addReportIdObj->addSpASINReportDownloadLink($storeArray);
                                // store report status
                                AMSModel::insertTrackRecord('report name : ASIN Report Link' . ' profile id: ' . $single->profileId, 'record found');
                            } else {
                                // store report status
                                AMSModel::insertTrackRecord('report name : ASIN Report Link' . ' profile id: ' . $single->profileId, 'not record found');
                            }
                        } catch (\Exception $ex) {
                            if ($ex->getCode() == 401) {
                                if (strstr($ex->getMessage(), '401 Unauthorized')) { // if auth token expire
                                    Log::error('Refresh Access token. In file filePath:Commands\Ams\ASIN\getReportLinkCron');
                            ...
                            // store report status
                            AMSModel::insertTrackRecord('ASIN Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\ASIN\getReportLinkCron. End Cron.");
    }
}
